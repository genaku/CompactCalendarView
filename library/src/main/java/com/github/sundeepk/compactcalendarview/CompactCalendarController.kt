...
internal class CompactCalendarController(
    ...
    fun onSingleTapUp(e: MotionEvent) {
        ...
        val dayColumn = Math.round((paddingLeft + e.x - paddingWidth.toFloat() - paddingRight.toFloat()) / widthPerDay)
    ...
    fun onScroll(e1: MotionEvent, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
        ...
        if (currentDirection == Direction.NONE) {
            currentDirection = if (Math.abs(distanceX) > Math.abs(distanceY)) {
    ...
    fun drawEvents(canvas: Canvas, currentMonthToDrawCalender: Calendar, offset: Int) {
        ...
        val uniqEvents = eventsContainer?.getEventsForMonthAndYear(currentMonth, currentMonthToDrawCalender.get(Calendar.YEAR))
        ...
        if (uniqEvents != null) {
            for (i in uniqEvents.indices) {
                ...
                val dayOfWeek = getDayOfWeek(eventsCalendar)

                val weekNumberForMonth = eventsCalendar.get(Calendar.WEEK_OF_MONTH)
                val xPosition = (widthPerDay * dayOfWeek).toFloat() + paddingWidth.toFloat() + paddingLeft.toFloat() + accumulatedScrollOffset.x + offset.toFloat() - paddingRight
    ...
    private fun drawTwoEvents(canvas: Canvas, xPosition: Float, yPosition: Float, eventsList: List<Event>) {
    ...
    private fun drawEventsWithPlus(canvas: Canvas, xPosition: Float, yPosition: Float, eventsList: List<Event>) {
    ...
    fun drawMonth(canvas: Canvas, monthToDrawCalender: Calendar, offset: Int) {
        ...
        val isSameMonthAsToday = monthToDrawCalender.get(Calendar.MONTH) == todayCalender.get(Calendar.MONTH)
        val isSameYearAsToday = monthToDrawCalender.get(Calendar.YEAR) == todayCalender.get(Calendar.YEAR)
        val isSameMonthAsCurrentCalendar = monthToDrawCalender.get(Calendar.MONTH) == currentCalender.get(Calendar.MONTH) && monthToDrawCalender.get(Calendar.YEAR) == currentCalender.get(Calendar.YEAR)
        val todayDayOfMonth = todayCalender.get(Calendar.DAY_OF_MONTH)
        ...
        while (dayColumn <= 6) {
            ...
            val xPosition = (widthPerDay * dayColumn).toFloat() + paddingWidth.toFloat() + paddingLeft.toFloat() + accumulatedScrollOffset.x + offset.toFloat() - paddingRight
