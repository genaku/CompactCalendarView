...
internal class AnimationHandler(
        ...
        private val compactCalendarView: CompactCalendarView
) {

    var isAnimating = false
        private set

    private var mCompactCalendarAnimationListener: CompactCalendarView.CompactCalendarAnimationListener? = null
    ...
    private val targetGrowRadius: Int
        get() {
            ...
            return (0.5 * Math.sqrt((heightSq + widthSq).toDouble())).toInt()
        }

    fun setCompactCalendarAnimationListener(compactCalendarAnimationListener: CompactCalendarView.CompactCalendarAnimationListener) {
        mCompactCalendarAnimationListener = compactCalendarAnimationListener
    }
    ...
    private fun getCollapsingAnimation(isCollapsing: Boolean): Animation =
            CollapsingAnimation(
                    ...
                    compactCalendarController.targetHeight,
                    targetGrowRadius,
                    isCollapsing
            )
    ...
    private fun onOpen() {
        mCompactCalendarAnimationListener?.onOpened()
    }

    private fun onClose() {
        mCompactCalendarAnimationListener?.onClosed()
    }
